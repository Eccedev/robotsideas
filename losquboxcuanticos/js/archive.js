...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const filtersSort = document.querySelector('.filters-sort');

    function updateFiltersVisibility() {
        // Si estamos en móvil (<931px), ocultar filtros y mostrar botón
        if (window.innerWidth < 931) {
            toggleBtn.style.display = 'block';
            // Si no está visible, asegúrate de que esté oculto
            if (!filtersSort.classList.contains('visible-on-mobile')) {
                filtersSort.style.display = 'none';
                toggleBtn.setAttribute('aria-expanded', 'false');
            }
        } else {
            // En escritorio, mostrar filtros y ocultar botón
            toggleBtn.style.display = 'none';
            filtersSort.style.display = '';
            filtersSort.classList.remove('visible-on-mobile');
            toggleBtn.setAttribute('aria-expanded', 'false');
        }
    }

    toggleBtn.addEventListener('click', function () {
        const isVisible = filtersSort.classList.toggle('visible-on-mobile');
        // Alternar display según la clase
        filtersSort.style.display = isVisible ? 'flex' : 'none';
        toggleBtn.setAttribute('aria-expanded', isVisible ? 'true' : 'false');
        toggleBtn.textContent = isVisible ? 'Ocultar Filtros' : 'Mostrar Filtros';
    });

    // Actualizar visibilidad al cargar y al cambiar tamaño de pantalla
